...
const projectColorMap = new Map<string | null, string>();

export function getProjectColor(projectPath: string | null): string {
  if (projectColorMap.has(projectPath)) {
    return projectColorMap.get(projectPath)!;
  }
  
  const color = PROJECT_COLORS[projectColorMap.size % PROJECT_COLORS.length];
  projectColorMap.set(projectPath, color);
  return color;
}
...
export const useTabStore = create<TabStore>((set, get) => ({
  ...
  addFileTab: (filePath: string, projectPath: string | null) => {
    // Check if a file tab for this path already exists — if so, switch to it
  ...
  addTasksTab: (projectPath: string) => {
    // Reuse existing tasks tab for this project if one exists
    const existing = get().tabs.find(t => t.type === 'tasks' && t.projectPath === projectPath);
    if (existing) {
      get().switchTab(existing.id);
      return existing.id;
    }
