...
export function useDefaultCommands() {
  ...
  useEffect(() => {
    ...
    const commandMeta: Record<string, { icon: string; action: () => void; keywords: string[] }> = {
      ...
      'new-tab':              { icon: 'Plus',           action: () => { if (!addTab()) openProjectDialog(); },    keywords: ['tab', 'new', 'create'] },
      'close-tab':            { icon: 'X',              action: () => { if (activeTabId) closeTab(activeTabId); }, keywords: ['tab', 'close', 'remove'] },
      'new-conversation':     { icon: 'MessageSquare',  action: () => { if (!addTab()) openProjectDialog(); },    keywords: ['new', 'conversation', 'chat', 'tab'] },
      'toggle-yolo-mode':     { icon: 'Zap',            action: () => { const pp = useProjectStore.getState().projectPath; if (activeTabId && pp) toggleYolo(activeTabId, pp); }, keywords: ['yolo', 'sandbox', 'auto', 'accept'] },
      ...
      'split-vertical':       { icon: 'Columns',         action: () => { const tabId = useTabStore.getState().activeTabId; if (tabId) useTabStore.getState().splitTabLayout(tabId, 'vertical'); },    keywords: ['split', 'vertical'] },
      'split-horizontal':     { icon: 'Rows',            action: () => { const tabId = useTabStore.getState().activeTabId; if (tabId) useTabStore.getState().splitTabLayout(tabId, 'horizontal'); },  keywords: ['split', 'horizontal'] },
      'unsplit':              { icon: 'PanelLeftClose',  action: () => { const tabId = useTabStore.getState().activeTabId; if (tabId) useTabStore.getState().collapseSplitLayout(tabId); },             keywords: ['split', 'close', 'unsplit'] },
